...
```text
数据采集层 (OPC UA Client) → AI分析层 (Isolation Forest) → 展示层 (Streamlit Web)
```
...
1. 下载并安装 Prosys OPC UA Simulation Server
2. 启动软件，确保状态显示为 **Running**
3. 记下连接地址（默认：`opc.tcp://localhost:53530/OPCUA/SimulationServer`）
...
![异常检测结果图](https://github.com/Cloud0426/opcua_project/raw/master/anomaly_detection_result.png)
